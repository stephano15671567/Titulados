...
const InformanteTable = () => {
  ...
  useEffect(() => {
    const fetchAssignmentsAndNotes = async () => {
      if (profesorId) {
        const assignmentsResponse = await axios.get(`10.100.32.192:4000/api/asignaciones/informante/${profesorId}`);
 ...
 const handleUpload = async () => {
  ...
  try {
    const response = await axios.post(`10.100.32.192:4000/api/archivos/subir/rubrica/informante/${alumnoRut}`, formData, {
      headers: {
  ...
  const handleDownload = () => {
    const alumnoRut = selectedAlumno.alumno_RUT;
    window.location.href = `10.100.32.192:4000/api/archivos/descargar/rubrica/informante`;
  };
