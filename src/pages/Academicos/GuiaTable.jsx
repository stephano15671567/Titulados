...
const GuiaTable = () => {
  ...
  useEffect(() => {
    const fetchAssignmentsAndNotes = async () => {
      if (profesorId) {
        const assignmentsResponse = await axios.get(`https://localhost:4000/api/asignaciones/guia/${profesorId}`);
        const notasResponse = await axios.get('https://localhost:4000/api/notas');
        const alumnosResponse = await axios.get('https://localhost:4000/api/alumnos');
  ...
  const handleSave = async () => {
    ...
    const url = `https://localhost:4000/api/notas/upsert`;
    const payload = {
...
const handleUpload = async () => {
  ...
  try {
    const response = await axios.post(`https://localhost:4000/api/archivos/subir/rubrica/guia/${alumnoRut}`, formData, {
      headers: {
  ...
  const handleDownload = () => {
    const alumnoRut = selectedAlumno.alumno_RUT;
    window.location.href = `https://localhost:4000/api/archivos/descargar/rubrica/guia`;
  };
  ...
  const handleUploadTesis = async () => {
    ...
    try {
      await axios.post(`https://localhost:4000/api/archivos/subir/tesis/${alumnoRut}`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
